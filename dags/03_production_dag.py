...
# ---------------------- NEO4J - Constraints ----------------------
# Creation of contraints in order to accelerate the creation of the graph
...
def create_participation_constraint():
    from neo4j import GraphDatabase
    ...
    with driver.session() as session:
        session.run("""
            CREATE CONSTRAINT participation_unique IF NOT EXISTS
            FOR (p:Participation)
            REQUIRE (p.athlete_id, p.edition_id, p.sport, p.event) IS UNIQUE;
        """)
# ---------------------- NEO4J - Nodes ----------------------
# Creation of all the nodes and the relationships
...
def create_participation_nodes():
    ...
    with driver.session() as session:
        ...
        CALL apoc.periodic.iterate(
          ...
          MERGE (p:Participation {
              athlete_id: row.athlete_id,
              edition_id: toInteger(row.edition_id),
              sport: row.sport,
              event: row.event
          })
          SET p.medal = CASE
...
def create_disaster_nodes():
    ...
    with driver.session() as session:
        session.run("""
            CALL apoc.periodic.iterate(
            "LOAD CSV WITH HEADERS FROM 'file:///Cleaned_Natural_Disasters.csv' AS row RETURN row",
            "
...
with DAG(
    ...
    create_participation_constraint_task = PythonOperator(
        task_id="create_participation_constraint",
        python_callable=create_participation_constraint
    )

    constraint_done = EmptyOperator(
        task_id="constraint_done",
   ...
   # ---- Constraints (parallel) ----
    init_neo4j >> [
        ...
        create_participation_constraint_task,
    ] >> constraint_done

    # ---- Base nodes ----
    constraint_done >> create_country_nodes_task >> create_athlete_nodes_task

    create_country_nodes_task >> create_edition_nodes_task

    constraint_done >> create_event_nodes_task

    # ---- Participation (fan-in) ----
    [
